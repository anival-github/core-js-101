...
function getCicleCircumference(radius) {
  return 3.141592653589793 * 2 * radius;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // Math.abs((Math.PI * angleDegrees) / 180);
...
function roundToPowerOfTen(num, pow) {
  const smallNum = num / (10 ** pow);
  const roundedSmallNum = Math.round(smallNum);
  const ruundedNum = roundedSmallNum * (10 ** pow);
  return ruundedNum;
}
